...
//筛选条件   返回
    ...
    public function getTcates($id='')
    {
         $where="p_id = $id";
         $cates = Db::name('garden_fen_biao')->where($where)->select();
    ...
    {
        $catesname =I('get.catesname');
        foreach($catesname as $k=>$v){
            $name = $v;
        }
        if(!empty($name)){
            $where = "name = $name";
            $res = Db::name('gardenlog_cates')->where($where)->find();
            $data=[];
            if($res){
                $msg = '次类别名称已有';
                echo  json_encode($msg);
            }else{
                $res = Db::name('gardenlog_cates')->insert($catesname);
                if($res){
                    $msg = '添加成功';
                    echo  json_encode($msg);
                }else{
